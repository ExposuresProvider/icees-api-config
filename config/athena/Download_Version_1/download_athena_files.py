...
def download_athena_files(search_list, name_list, url, url2, outfile):
    ...
    for iter_num, feature in enumerate(search_list):
        ...
        # if we still got 400 error because the file was large then use all domain and all vocab to download files
        ...
        if os.path.isfile(file_name):
            if os.stat(file_name).st_size == 58:
                ...
                for d in domain:
                  ...
                  ## Concatenate all files with domain and vocab cominbation into one files for all features
